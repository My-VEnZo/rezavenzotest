...
function handleCallbackQuery($callback_query) {
    ...
    if (strpos($data, 'toggle_prevent_unlimited_traffic:') === 0) {
        ...
        $page = 1; 
        if (preg_match('/manage_admins:page:(\d+)/', $data, $matches)) {
        ...
        $pagedAdmins = array_slice($admins, $start, $perPage);
        ...
        $keyboard['inline_keyboard'] = array_merge($keyboard['inline_keyboard'], $pagedAdmins);

        if ($totalAdmins > $perPage) {
            $paginationButtons = [];
            if ($page < $totalPages) {
                $paginationButtons[] = ['text' => $lang['next_page'], 'callback_data' => 'manage_admins:page:' . ($page + 1)];
            } else {
                $paginationButtons[] = ['text' => $lang['next_page'], 'callback_data' => 'noop'];
            }
            if ($page > 1) {
                $paginationButtons[] = ['text' => $lang['previous_page'], 'callback_data' => 'manage_admins:page:' . ($page - 1)];
            } else {
                $paginationButtons[] = ['text' => $lang['previous_page'], 'callback_data' => 'noop'];
            }
        ...
        sendRequest('editMessageText', [
            'chat_id' => $chatId,
            'message_id' => $messageId,
            'text' => sprintf($lang['select_admin_prompt'], $page, $totalPages),
            'parse_mode' => 'Markdown',
            'reply_markup' => json_encode($keyboard)
        ]);
        return;
